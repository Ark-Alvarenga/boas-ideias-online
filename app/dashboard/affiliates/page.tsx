...
import { Lightbulb, ArrowUpRight, Share2 } from "lucide-react"
...
async function getAffiliateData(userId: ObjectId) {
  ...
  const [clicksAgg, salesAgg] = await Promise.all([
    db.collection<AffiliateClick>("affiliateClicks").aggregate<{ _id: ObjectId; count: number }>([
      { $match: { affiliateId: { $in: affiliateIds } } },
      { $group: { _id: "$affiliateId", count: { $sum: 1 } } },
    ]).toArray(),
    db.collection<AffiliateSale>("affiliateSales").aggregate<{ _id: ObjectId; count: number; total: number }>([
      { $match: { affiliateId: { $in: affiliateIds } } },
      { $group: { _id: "$affiliateId", count: { $sum: 1 }, total: { $sum: "$commissionAmount" } } },
    ]).toArray(),
  ])
        ...
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6 lg:px-8">
          ...
          <div className="flex items-center gap-3">
            <Button variant="ghost" size="sm" asChild>
              <Link href="/dashboard/affiliate-earnings">Ganhos</Link>
            </Button>
          ...
          <CardContent>
            ...
            {rows.length === 0 && (
              <p className="py-8 text-center text-sm text-muted-foreground">
                Você ainda não é afiliado de nenhum produto. Encontre produtos com programa de afiliados no{" "}
