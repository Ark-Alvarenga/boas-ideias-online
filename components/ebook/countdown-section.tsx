  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      if (distance > 0) {
        ...
        // Reinicia automaticamente em 15 min
        const newTarget = new Date(Date.now() + 15 * 60 * 1000);
      ...
      <div className="max-w-6xl mx-auto px-4">
        <div className="flex flex-col lg:flex-row items-center justify-between gap-8">
          ...
          <div className="flex gap-3 md:gap-4">
            {[
              { value: time.hours, label: "HOR" },
              { value: time.minutes, label: "MIN" },
              { value: time.seconds, label: "SEG" },
            ].map((item, index) => (
              <div key={index} className="text-center">
                <div
                  className={`text-2xl md:text-5xl font-bold w-14 md:w-20 h-14 md:h-20 flex items-center justify-center rounded-lg shadow-lg transition-all ${
                    isEnding
                      ? "bg-red-500 text-white shadow-red-500/40 animate-pulse"
                      : "bg-cyan-500 text-gray-900 shadow-cyan-500/20"
                  }`}
                >
        ...
        <div className="text-center mt-8">
          <p className="text-red-400 text-sm font-semibold">
            ⚠️ Quanto mais você espera, mais tempo você perde sem ganhar
            dinheiro online
          </p>
        </div>
      </div>
